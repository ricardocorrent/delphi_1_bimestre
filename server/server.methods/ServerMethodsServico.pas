...
begin
  try
  fdQueryDeleteServico.Active := False;
  fdQueryDeleteServico.ParamByName('ID').AsInteger := AId;
  fdQueryDeleteServico.ExecSQL;
except
    raise Exception.Create('Falha ao excluir serviço');
end;
end;

function TsmServico.GetAllServico: TFDJSONDataSets;
begin
  fdQueryAllServico.Active := False;
  Result := TFDJSONDataSets.Create;
  TFDJSONDataSetsWriter.ListAdd(Result, fdQueryAllServico);
end;
